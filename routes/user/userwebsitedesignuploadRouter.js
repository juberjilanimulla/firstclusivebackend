...
const websitedesignuploadimageRouter = Router();

websitedesignuploadimageRouter.post("/:id", async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(websiteid)) {
        ...
        return errorResponse(res, 400, "Invalid website  ID");
      }
      ...
      for (const file of req.files) {
        const result = await cloudinary.uploader.upload(file.path, {
          folder: "firstclusive/websitedesign-images",
        });
      ...
      const websitedesign = await websitedesignmodel.findByIdAndUpdate(
        websiteid,
      ...
      if (!websitedesign)
        return errorResponse(res, 404, "website design not found");
...
export default websitedesignuploadimageRouter;
